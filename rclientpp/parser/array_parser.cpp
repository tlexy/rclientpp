...
int ArrayParser::parse(const std::shared_ptr<RClientBuffer> raw_resp, std::shared_ptr<BaseValue>& result)
{
	...
	const int ret = parse_array(raw_resp, len, complex_result);
	if (ret < 0)
	{
		//raw_resp->set_read_off(old_pos);
		return ret;
